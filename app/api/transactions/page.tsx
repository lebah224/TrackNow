...
export default async function NewTransactionPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  return <main className="main" style={{maxWidth:650}}>
    <div className="card" style={{marginTop:'7vh'}}>
      <div className="eyebrow">TRACKNOW · MOUVEMENT</div>
      <h1 style={{fontSize:28,margin:'6px 0'}}>Enregistrer {type === 'income' ? 'un revenu' : 'une dépense'}</h1>
      ...
      <div style={{display:'flex',gap:8,marginTop:14}}><a className="btn" href="/api/transactions?type=income">+ Revenu</a><a className="btn" href="/api/transactions?type=expense">− Dépense</a></div>
    </div>
  </main>
}
