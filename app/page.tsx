...
export default async function Home() {
  ...
  return <div className="app">
    ...
    <main className="main">
      <header className="top"><div><div className="eyebrow">Tableau de bord</div><div className="title">Bonjour, {displayName} 👋</div></div><Link className="btn primary" href="/transactions">+ Mouvement</Link></header>
      ...
      <section className="grid three" style={{ marginTop: 16 }}>
        ...
        <div className="card"><div className="eyebrow">Épargne calculée</div><div className={`value ${saving >= 0 ? 'good' : 'bad'}`} style={{ marginTop: 7 }}>{money(saving)}</div></div>
      </section>
      <section className="grid two" style={{ marginTop: 16 }} id="argent">
        <div className="card"><div className="cardhead"><h2>Comptes</h2><span className="chip">{accounts?.length ?? 0}</span></div>{(accounts ?? []).length === 0 ? <p className="muted">Aucun compte. <Link href="/onboarding">Ajoute ton premier compte</Link> pour que TrackNow puisse calculer ton patrimoine.</p> : accounts?.map(a => <div className="stat" key={a.id}><span>{a.name}</span><b>{money(Number(a.opening_balance))}</b></div>)}</div>
        <div className="card"><div className="cardhead"><h2>Derniers mouvements</h2><span className="chip">Chaque franc compte</span></div>{(transactions ?? []).length === 0 ? <p className="muted">Aucune transaction enregistrée. <Link href="/transactions">Ajouter un mouvement</Link></p> : transactions?.map(t => <div className="tx" key={t.id}><div className="txicon">{t.type === 'income' ? '💼' : '🧾'}</div><div className="txmain"><b>{t.description || (t.type === 'income' ? 'Revenu' : 'Dépense')}</b><span>{new Date(t.occurred_at).toLocaleDateString('fr-FR')}</span></div><div className={`amount ${t.type === 'income' ? 'good' : 'bad'}`}>{t.type === 'income' ? '+' : '−'}{money(Number(t.amount))}</div></div>)}</div>
      </section>
      <section className="card" style={{ marginTop: 16 }} id="progression"><div className="cardhead"><h2>🎯 Objectifs actifs</h2><span className="chip">{goals?.length ?? 0}</span></div>{(goals ?? []).length === 0 ? <p className="muted">Crée ton premier objectif financier pour commencer à mesurer ta progression.</p> : goals?.map(g => { const target = Number(g.target_amount); const current = Number(g.current_amount); const pct = target > 0 ? Math.min(100, Math.max(0, current / target * 100)) : 0; return <div key={g.id} style={{ marginBottom: 14 }}><div className="stat"><span>{g.name}</span><b>{money(current)} / {money(target)}</b></div><div className="progress"><i style={{ width: `${pct}%` }} /></div></div> })}</section>
